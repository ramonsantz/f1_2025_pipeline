...
def run_scraper():
    ...
    # Caminho local (relativo ao projeto)
    output_path_local = os.path.join('data', 'raw', 'fastest_laps_2025.csv')
    df_fastest.to_csv(output_path_local, index=False)

    print(f"[OK] Fastest laps salvo em {output_path_local}")

# Executa quando o script é chamado diretamente
